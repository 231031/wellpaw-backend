...
type FoodController interface {
	...
	GetFoodByIDAndUserID(ctx *fiber.Ctx) error
...
func (c *foodController) GetFoodByIDAndUserID(ctx *fiber.Ctx) error {
	userID := ctx.Locals("id").(uint)
	...
	response := c.foodService.GetFoodByIDAndUserID(ctxWithTimeout, userID, uint(foodID64))
	return ctx.Status(response.Status).JSON(response)
}
