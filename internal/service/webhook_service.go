...
func (s *webhookService) HandleSubscriptionFreeTierUpdated(ctx context.Context, eventObject *stripe.Event) error {
	...
	if err := s.userRepository.UpdateSubscriptionDetail(ctx, invoice.Customer.ID, model.ACTIVESUB, model.FREE); err != nil {
		applogger.LogError(fmt.Sprintf("webhook handler failed to update subscription free-tial detail : %s", err.Error()), serviceLog)
		return err
	}

	err = s.userRepository.SetCurrentSubscriptionDetail(ctx, invoice.Customer.ID, model.FREE, model.ACTIVESUB)
	if err != nil {
